...
public class Testing {
    ...
    public void setup() {
        ...
        admin = new Admin("Admin", "User", "admin123", "adminpass", "test", "0000");
    ...
    public void testAdminUserCreation() {
        User fetchedAdmin = library.validateUser("admin123", "adminpass");
        assertNotNull(fetchedAdmin, "Admin should be created and validated successfully");
        assertTrue(fetchedAdmin instanceof Admin, "Fetched user should be an instance of Admin");
    }
/*
    @Test
    public void testUserActivationAndDeactivation() {
        User user = new User("John", "Doe", "12345", "password", "john.doe@example.com", "555-5555");
        library.addUser(user);

        // Deactivate the user
        library.deactivateUser("John");
        assertFalse(user.isActive(), "User should be deactivated");

        // Check that inactive user cannot log in
        User deactivatedUser = library.validateUser("12345", "password");
        assertNull(deactivatedUser, "Deactivated user should not be able to log in");

        // Activate the user
        library.activateUser("John");
        assertTrue(user.isActive(), "User should be activated");

        // Check that active user can log in
        User activatedUser = library.validateUser("12345", "password");
        assertNotNull(activatedUser, "Activated user should be able to log in");
    }
*/

    @Test
    public void testUserCannotLoginIfInactive() {
        User user = new User("Inactive", "User", "55555", "password", "inactive.user@example.com", "555-1234");
        library.addUser(user);

        // Deactivate the user
        library.deactivateUser("Inactive");

        // Attempt to log in with an inactive account
        User invalidLogin = library.validateUser("55555", "password");
        assertNull(invalidLogin, "Inactive user should not be able to log in");
    }
